...
int transfer_builtup_arrays(variable *var);
int transfer_arrays(variable *var, MLVars *ml_struct);
...
static int do_error(FILE *, variable **vectors, char *, int, MLVars *, char *,
		    char *);
static int do_float(FILE *, variable **vectors, char *, int, MLVars *, char *,
		    char *);
static int do_string(FILE *, variable **vectors, char *, int, MLVars *, char *,
		     char *);
static int do_list(FILE *, variable **vectors, char *, int, MLVars *, char *,
		   char *);
static int do_array(FILE *, variable **vectors, char *, int, MLVars *, char *,
		    char *);
...
static int do_sequence(FILE *, variable **vectors, char *, int, MLVars *,
		       char *, char *);
static int do_grid(FILE *, variable **vectors, char *, int, MLVars *, char *,
		   char *);
static int do_attributes(FILE *, variable **vectors, char *, int, MLVars *,
			 char *, char *);

typedef int (*reader_type)(FILE *, variable **vectors, char *, int, MLVars *,
			   char *, char *);
...
struct typeitem {
    char typename[64];
    reader_type do_type;
} typelist[] = { { "Error", do_error }, { "Byte", do_float }, { "Int16",
								do_float }, { "UInt16", do_float }, { "Int32", do_float }, { "UInt32",
															     do_float }, { "Float32", do_float }, { "Float64", do_float }, {
	"String", do_string }, { "Url", do_string }, { "List", do_list }, {
	"Array", do_array }, { "Structure", do_structure }, { "Sequence",
							      do_sequence }, { "Function", do_sequence }, { "Grid", do_grid }, {
	"Attributes", do_attributes }, { "", NULL } };
...
static void build_variable_name_vector() {
    char *name;
    char **names, **names_tmp;
    size_t number_of_vars = (size_t)get_number_of_variables();

    names = (char **)mxCalloc(number_of_vars, sizeof(char *));DBGM(msg("mxCalloc (%s:%d): %x\n", __FILE__, __LINE__, names));
...
static void strip(char *dest, char *src, char c) {
    while (*src) {
	if (*src == c)
	    *dest = *src++;
	else
...
static reader_type get_reader(char *typename) {
    size_t k = 0;
    do {
...
static int read_name(FILE *fin, char name[MAX_STR], int verbose) {
    size_t items;
    char line[256];
...
static int read_vec_descript(FILE *fin, char element_type[MAX_STR],
			     char name[MAX_STR], int *length) {
    size_t items;
    char line[256];
    ...
    items = sscanf(line, "%s %s %d", &element_type[0], &name[strlen(name)], length);
    if (items !=3) {
	err_msg(\
...
static int read_array_dims(FILE *fin, int ndims, int *dims) {
	...
	if (i > ndims) {
	    err_msg(
		    \
		    "Internal Error: Confusion reading array dimensions (%s:%d)",
		    __FILE__,
		    __LINE__);
	    return FALSE;
	}
...
static int read_aggregate(FILE *fin, char name[MAX_STR], int *length) {
    size_t items;
    char line[256];
    fgets(line, 255, fin);
...
static int read_trailing_newline(FILE *fin) {
    char dummy[256];
    if (feof(fin)) {
...
static int do_error(FILE *fin, variable **vectors, char *prefix, int outermost,
...
static int do_float(FILE *fin, variable **vectors, char *prefix, int outermost,
		    MLVars *vars, char *child, char *parent) {
    double y;
    char name[MAX_STR] = "";
    variable *var;
...
static char * read_string_value(FILE *fin) {
    int len;
    int is_quoted= FALSE;
    int first_line= TRUE;
    char *value= NULL;
    ...
    *(value + len) = '\0';
    ...
    /* The test for !IS_END_NEWLINE covers strings larger than MAX_STR.
       ...
       jhrg */
    while ((is_quoted) ? !IS_END_QUOTE(lbuf, len, first_line) : !IS_END_NEWLINE(lbuf, len)) {
	first_line = FALSE;
	fgets(lbuf, BIG_STR-1, fin);
	len = strlen(lbuf);
...
static int do_string(FILE *fin, variable **vectors, char *prefix,
...
void print_array_creation_msg(char *name, int num_elems, int ndims, int dims[]) {
    switch (ndims) {
      case 0:
...
static int do_array(FILE *fin, variable **vectors, char *prefix, int outermost,
		    MLVars *vars, char *child, char *parent) {
    size_t i;
    int ndims;
    int *dims;
    ...
    dims = mxCalloc((size_t)ndims, sizeof(int));DBGM(msg("mxCalloc (%s:%d): %x\n", __FILE__, __LINE__, dims));
    ...
    if (strcmp(element_type, "Byte") == 0 || strcmp(element_type, "Int16") == 0
	|| strcmp(element_type, "UInt16") == 0 || strcmp(element_type,
							 "Int32") == 0 || strcmp(element_type, "UInt32") == 0 || strcmp(
															element_type, "Float32") == 0 || strcmp(element_type, "Float64")
	== 0) {
    ...
    } else { /* Do this for non-numeric arrays. */
	int status;
...
static int do_list(FILE *fin, variable **vectors, char *prefix, int outermost,
    ...
    /* Process arrays of simple numeric types specially. */
    if (strcmp(element_type, "Byte") == 0 || strcmp(element_type, "Int32") == 0
...
static int do_structure(FILE *fin, variable **vectors, char *prefix,
			int outermost, MLVars *vars, char *child, char *parent) {
    int num_elems;
    int status;
    size_t i;
...
/** @param vars Ignored. */
static int do_sequence(FILE *fin, variable **vectors, char *prefix,
		       int outermost, MLVars *vars, char *child, char *parent) {
    int num_elems;
    char element_type[MAX_STR];
    reader_type exectype;
...
/** @param vars Ignored. */
static int do_grid(FILE *fin, variable **vectors, char *prefix, int outermost,
		   MLVars *vars, char *child, char *parent) {
    char name[MAX_STR] = ""; /* PREFIX + short_name */
    char cName[MAX_STR] = "";
    int num_elems;
	...
	for (j = 0; j < num_elems; ++j) {
	    ...
	    status
		= do_array(fin, vectors, name, FALSE, grid_vars, cName,
			   NULL);
	    if (!status) {
		return FALSE;
	    }
