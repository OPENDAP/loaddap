...
static char * quote_for_shell(char *src) {
	char *c;
	...
	*dest = '\0';
	...
	DBG(msg("quote_for_shell (%s:%d) src: %s\n", __FILE__, __LINE__, src));DBG(msg("quote_for_shell (%s:%d) dest: %s\n", __FILE__, __LINE__, dest));
 ...
 */
static char * get_pathname(char *command) {
...
/** Read the command arguments and perform initial actions. If the command is
 ...
 required. */
static char * init(int nlhs, mxArray *plhs[], const int nrhs,
		const mxArray *prhs[]) {
...
"Error: Could not find the loaddap helper application (writedap).\n\
       ...
       MATLABPATH).\n");
		...
		command = mxCalloc(strlen(command_opts) + strlen(pathname)
				+ strlen("2>&1") + strlen(DODS_COMPLETE_COMMAND) + 1,
				sizeof(char));DBGM(msg("mxCalloc (%s:%d): %x\n", __FILE__, __LINE__, command));
...
#else
			sprintf(command, DODS_COMPLETE_COMMAND, pathname, command_opts,
					"2>&1");DBG(msg("command: %s\n", command));
			fin = popen(command, "r");
#endif
			if (fin) {
				fgets(ver, MAX_STR, fin);DBG(msg("Writedap version string: %s\n", ver));
				pclose(fin);
				strcat(ver, "loaddap: ");
...
"Error: Could not find the loaddap helper application (writedap).\n\
	...
	command = mxCalloc(strlen(pathname) +strlen(dods_url_escaped)
			+ strlen(command_opts) + strlen(DODS_COMPLETE_COMMAND) + 1,
			sizeof(char));DBGM(msg("mxCalloc (%s:%d): %x\n", __FILE__, __LINE__,
					command));

	sprintf(command, DODS_COMPLETE_COMMAND, pathname, command_opts,
			dods_url_escaped);DBG(msg("command: %s\n", command));
...
"Internal Error: loaddap found its helper application (writedap), but\n\
                ...
                Otherwise, contact support at opendap.org.\n");
		goto exit;
	}
